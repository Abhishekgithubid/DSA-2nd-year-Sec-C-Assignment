...
class queue
{
	...
	bool isempty()                                 //TO CHEAK EMPTY OR NOT
	{
		if(front==-1&&rear==-1)
		{
			return true;
	...
	bool isfull()                                   //TO CKEAK FULL OR NOT 
	{
		if(rear==4)
		{
			return true;
	...
	void Enqueue(int val )                         //TO ENTER  A ELEMENT IN THE QUEUE
	{ 
	...
	int dequeue()                                          // DELETE ALEMENT  FROM THE QUEUE
	{
		int x;
		if(isempty())
		{
	...
	void disply()                                           // FOR DISPLY THE ALEMENT 
	{
